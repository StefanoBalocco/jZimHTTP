...
async function writeLog( entry: Parameters<RequestLogger['log']>[0] ): Promise<string> {
	const logPath: string = join( tmpdir(), `test-clf-${ randomUUID() }.log` );
	const logger: RequestLogger = new RequestLogger( logPath );
	logger.log( entry );
	await new Promise<void>( resolve => { logger.shutdown( resolve ); } );
	const content: string = readFileSync( logPath, 'utf-8' ).trimEnd();
	unlinkSync( logPath );
	return content;
}

test( 'log produces valid CLF line', async ( t ) => {
	const line: string = await writeLog( {
...
test( 'RequestLogger writes CLF lines to file', async ( t ) => {
	const logPath: string = join( tmpdir(), `test-access-${ Date.now() }.log` );
	const logger: RequestLogger = new RequestLogger( logPath );
	logger.log( {
		remoteHost: '10.0.0.1',
	...
	await new Promise<void>( resolve => { logger.shutdown( resolve ); } );
	const content: string = readFileSync( logPath, 'utf-8' );
