...
// Helper: create a temp directory with optional empty stub files
...
// --- ResolveArchivePath (path traversal) ---

test( 'path traversal: returns null on ../escape', async t => {
	const dir = await makeTempDir();
	const zim: Zim = new Zim( dir, 4, 200, 300000 );
	const mcp: Mcp = new Mcp( zim, mcpConfig, 200, 300000 );
	try {
...
// --- Real ZIM file integration tests ---

const _zimDataDir: string = path.join( process.cwd(), 'tests/data' );
const _zimFileName: string = 'wikipedia_en_100_mini_2026-01.zim';

test( 'Search with real file: returns results for a common query', async t => {
	const zim: Zim = new Zim( _zimDataDir, 4, 200, 300000 );
	const mcp: Mcp = new Mcp( zim, mcpConfig, 200, 300000 );
	try {
...
test( 'Article with real file: returns full markdown for a search result', async t => {
	...
	try {
		...
		const firstPath: string = searchResult.results[ 0 ]!.path;
...
test( 'Mcp.Search paginates via page without fetching 200 rows upfront', async t => {
	...
	try {
		...
		if( 0 < page1.results.length && 0 < page2.results.length ) {
			const p1Paths: Set<string> = new Set( page1.results.map( r => r.path ) );
			const overlap: number = page2.results.filter( r => p1Paths.has( r.path ) ).length;
