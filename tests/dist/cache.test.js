...
test('Get returns undefined for missing key', t => {
    const cache = new LruTtlCache(10, 60000);
...
test('DeleteByPrefix removes matching entries and returns count', t => {
    const cache = new LruTtlCache(100, 60000);
    ...
    const deleted = cache.deleteByPrefix('search:wiki_en.zim:');
    t.is(deleted, 2);
    t.is(cache.size, 2);
...
test('DeleteByPrefix with filename prefix removes all entry types for that file', t => {
    ...
    const deleted = cache.deleteByPrefix('wiki_en.zim');
    t.is(deleted, 3);
    t.is(cache.size, 1);
